...
if (isset($_POST['add_id']) && isset($_POST['qty']) && isset($_POST['product_name']) && isset($_POST['price'])) {
    ...
    echo json_encode(array("code" => 1, "count_cart" => count($_SESSION['cart']), 'htmlData' => $html, 'cart_total' => '<b>Total:</b> K' . number_format($cart_total, 2)));
...
//        print_r($_SESSION['cart']);return;
    for ($i = 0; $i < count($_SESSION['cart']); $i++) {

        if (isset($_SESSION['cart'][$prod_id])) {
            unset($_SESSION['cart'][$prod_id]); //remove old item
